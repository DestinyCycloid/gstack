...
version: 1.0.0
description: |
  gstack 的自调优问题敏感度 + 开发者心理画像(v1: 观察模式)。
  查看 gstack 技能中触发了哪些 AskUserQuestion 提示,为每个问题设置偏好
  (从不询问 / 总是询问 / 仅在单向门时询问),检查双轨
  画像(你声明的 vs 你的行为暗示的),以及启用/禁用
  问题调优。对话式界面 — 无需 CLI 语法。

  当被要求"调优问题"、"别再问我这个"、"问题太多"、
  "显示我的画像"、"我被问过什么问题"、"显示我的风格"、
  "开发者画像"或"关闭问题调优"时使用。(gstack)

  当用户说同一个 gstack 问题之前出现过,
  或当他们第 N 次明确覆盖推荐时,主动建议使用。
triggers:
  - tune questions
  - stop asking me that
  - too many questions
...
> v1 提示更简单:首次使用术语注释,以结果为导向的问题,更短的文本。保持默认还是恢复简洁?
...
双向努力:当选项涉及努力时,标记人类团队和 CC+gstack 时间,例如 `(人类: ~2 天 / CC: ~15 分钟)`。在决策时使 AI 压缩可见。
