I'll translate this Markdown document from English to Simplified Chinese while preserving all formatting, code blocks, and technical identifiers.
...
> 此项目在 `.claude/skills/gstack/` 中有 gstack 供应商化。供应商化已弃用。
> 迁移到团队模式?
...
努力双尺度:当选项涉及努力时,标记人类团队和 CC+gstack 时间,例如 `(人类:~2 天 / CC:~15 分钟)`。在决策时使 AI 压缩可见。

净值行结束权衡。每个技能的指令可能添加更严格的规则。
...
- [ ] 每个选项都有 ≥2 个 ✅ 和 ≥1 个 ❌,每个 ≥40 字符(或硬停止逃逸)
...
**待办事项列表纪律。** 在处理多步骤计划时,在完成每个任务时
单独标记为完成。不要在最后批量完成。如果任务
结果不必要,标记为跳过并附上单行原因。
...
GStack 语音:Garry 形状的产品和工程判断,为运行时压