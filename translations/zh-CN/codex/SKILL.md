...
version: 1.0.0
description: |
  OpenAI Codex CLI 包装器 — 三种模式。代码审查:通过 codex review 进行独立的 diff 审查,带通过/失败门控。挑战:对抗模式,尝试破坏你的代码。咨询:向 codex 询问任何问题,支持会话连续性以便后续跟进。"200 IQ 自闭症开发者"第二意见。当被要求"codex review"、"codex challenge"、"ask codex"、"second opinion"或"consult codex"时使用。(gstack)
  语音触发器(语音转文字别名):"code x"、"code ex"、"get another opinion"。
triggers:
  - codex review
  - second opinion
...
**当两个变体都不可调用时的回退:**在计划模式下,将决策简报作为 `## Decisions to confirm` 部分写入计划文件 + ExitPlanMode(原生"准备执行?"会显示它)。在计划模式之外,将简报作为散文输出并停止。**永远不要静默自动决策** — 只有 `/plan-tune` AUTO_DECIDE 选择授权自动选择。
...
Stakes if we pick wrong: <关于什么会坏、用户会看到什么、会失去什么的一句话>
...
努力双尺度:当选项涉及努力时,标记人类团队和 CC+gstack 时间,例如 `(human: ~2 days / CC: ~15 min)`。使 AI 压缩在决策时可见。
...
- [ ] 每个选项都有 ≥2 ✅ 和 ≥1 ❌,每个 ≥40 字符(或硬停止转义)
- [ ] (recommended) 标签在一个选项上(即使是中立姿态)
- [ ] 涉及努力的选项上的双尺度努力标签(人类 / CC)
- [ ] Net 行结束决策
- [ ] 你正在调用工具,而非写散文
...
以下微调针对 claude 模型系列进行了调整。它们**从属于**技能工作流、STOP 点、AskUserQuestion 门控、计划模式安全和 /ship 审查门控。如果下面的微调与技能指令冲突,技能获胜。将这些视为偏好,而非规则。

**待办事项列表纪律。**在处理多步骤计划时,在完成每个任务时单独标记为完成。不要在最后批量完成。如果任务证明是不必要的,用单行原因标记为跳过。

**在重操作前思考。**对于复杂操作(重构、迁移、非平凡的新功能),在执行前简要说明你的方法。这让用户可以廉价地纠正方向,而不是在飞行中。
