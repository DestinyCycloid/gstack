...
version: 1.0.0
description: |
  部署后的金丝雀监控。使用浏览守护进程监控线上应用的控制台错误、性能回归和页面故障。定期截图，与部署前的基线对比，并在发现异常时发出警报。适用场景："监控部署"、"金丝雀"、"部署后检查"、"监控生产环境"、"验证部署"。(gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
**待办事项列表纪律。**在执行多步骤计划时,在完成每个任务时单独标记为完成。不要在最后批量完成。如果任务被证明是不必要的,标记为跳过并附上单行原因。
