...
version: 1.0.0
description: |
  发布后文档更新。读取所有项目文档，交叉引用差异，更新 README/ARCHITECTURE/CONTRIBUTING/CLAUDE.md 
  以匹配已发布内容，润色 CHANGELOG 语气，清理 TODOS，并可选地升级 VERSION。当被要求"更新文档"、
  "同步文档"或"发布后文档"时使用。在 PR 合并或代码发布后主动建议。(gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
ELI10 始终存在，使用简单英语，而非函数名。推荐始终存在。保留 `(recommended)` 标签；AUTO_DECIDE 依赖于它。
...
**待办事项列表纪律。** 在处理多步骤计划时，在完成每个任务时单独标记完成。不要在最后批量完成。如果任务证明是不必要的，用单行原因标记为跳过。
