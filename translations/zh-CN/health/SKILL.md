...
version: 1.0.0
description: |
  代码质量仪表板。封装现有项目工具（类型检查器、代码检查器、
  测试运行器、死代码检测器、Shell 检查器），计算加权综合
  0-10 分数，并跟踪随时间变化的趋势。使用场景："健康检查"、
  "代码质量"、"代码库健康状况如何"、"运行所有检查"、
  "质量分数"。(gstack)
triggers:
  - code health check
  - quality dashboard
...
在计划模式下，允许以下操作因为它们为计划提供信息：`$B`、`$D`、`codex exec`/`codex review`、写入 `~/.gstack/`、写入计划文件，以及为生成的产物执行 `open`。
...
> v1 提示更简单：首次使用术语解释、以结果为导向的问题、更短的文本。保持默认还是恢复简洁模式？
...
如果 `LAKE_INTRO` 是 `no`：说"gstack 遵循 **Boil the Lake** 原则 — 当 AI 使边际成本接近零时，做完整的事情。了解更多：https://garryslist.org/posts/boil-the-ocean" 提供打开：
...
每个项目仅发生一次。如果 `HAS_ROUTING` 是 `yes` 或 `ROUTING_DECLINED` 是 `true` 则跳过。
...
每个 AskUserQuestion 都是一个决策简报，必须作为 tool_use 发送，而不是文本。

```
D<N> — <单行问题标题>
项目/分支/任务：<使用 _BRANCH 的 1 个简短定位句子>
ELI10：<16 岁的人能理解的简单英语，2-4 句话，说明利害关系>
选错的风险：<关于什么会损坏、用户会看到什么、会失去什么的一句话>
推荐：<选择> 因为 <单行原因>
完整性：A=X/10，B=Y/10   (或：注意：选项在类型上不同，而非覆盖范围 — 无完整性分数)
优缺点：
A) <选项标签>（推荐）
  ✅ <优点 — 具体、可观察、≥40 字符>
  ❌ <缺点 — 诚实、≥40 字符>
...
ELI10 始终存在，使用简单英语，而不是函数名。推荐始终存在。保留 `(recommended)` 标签；AUTO_DECIDE 依赖于它。
...
**在重大操作前思考。** 对于复杂操作（重构、迁移、非平凡的新功能），在执行前简要说明你的方法。这让用户可以廉价地纠正方向，而不是在进行中。
