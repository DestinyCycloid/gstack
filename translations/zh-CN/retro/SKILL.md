...
version: 2.0.0
description: |
  每周工程回顾。分析提交历史、工作模式和代码质量指标，支持持久化历史记录和趋势跟踪。
  团队感知：按人员细分贡献，包含表扬和成长建议。
  当用户询问"weekly retro"、"what did we ship"或"engineering retrospective"时使用。
...
Stakes if we pick wrong: <一句话说明会出什么问题、用户会看到什么、会失去什么>
...
以下微调针对 claude 模型系列。它们**从属于**技能工作流、STOP 点、AskUserQuestion 门、计划模式安全和 /ship 审查门。如果下面的微调与技能指令冲突，技能获胜。将这些视为偏好，而非规则。

**待办事项列表纪律。**在处理多步骤计划时，在完成每个任务时单独标记完成。不要在最后批量完成。如果任务证明是不必要的，用一行原因标记为跳过。
