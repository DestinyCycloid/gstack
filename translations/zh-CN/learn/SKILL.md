...
version: 1.0.0
description: |
  管理项目学习记录。查看、搜索、清理和导出 gstack 在多个会话中学到的内容。
  当用户询问"我们学到了什么"、"显示学习记录"、"清理过时的学习记录"或"导出学习记录"时使用。
  当用户询问过去的模式或疑惑"我们之前不是修复过这个吗?"时主动建议使用。
triggers:
  - show learnings
...
**待办事项列表纪律。**在执行多步骤计划时,在完成每个任务时单独标记完成。不要在最后批量完成。如果任务被证明是不必要的,用单行原因标记为跳过。
