I'll translate this Markdown document from English to Simplified Chinese while preserving all formatting, code blocks, and technical terms.
...
version: 2.0.0
description: |
  YC Office Hours — 两种模式。创业模式：六个强制性问题，揭示需求现实、现状、绝望的具体性、最窄楔子、观察和未来适配。构建者模式：为副项目、黑客马拉松、学习和开源项目进行设计思维头脑风暴。保存设计文档。
  当被要求"头脑风暴这个"、"我有个想法"、"帮我思考这个"、"办公时间"或"这值得构建吗"时使用。
  当用户描述新产品想法、询问某事是否值得构建、想要在任何代码编写之前思考设计决策或探索概念时，主动调用此技能（不要直接回答）。
...
> 让 gstack 主动建议技能，比如对"这能工作吗？"使用 /qa 或对错误使用 /investigate？
...
Stakes if we pick wrong: <一句话说明会破坏什么、用户会看到什么、会失去什么>
Recommendation: <选择> because <单行原因>
...
Pros / cons：使用 ✅ 和 ❌。当选择是真实的时，每个选项至少 2 个优点和 1 个缺点；每个要点至少 40 个字符。单向/破坏性确认的硬停止逃生：`✅ No cons — this is a hard-stop choice`。

中立姿态：`Recommendation: <default> — this is a taste call, no strong preference either way`；`(recommended)` 保留在默认选项上以供 AUTO_DECIDE 使用。

Effort both-scales：当选项涉及工作量时，标记人类团队和 CC+gstack 时间，例如 `(human: ~2 days / CC: ~15 min)`。使 AI 压缩在决策时可见。
...
- [ ] 每个选项都有 ≥2 ✅ 和 ≥1 ❌，每个 ≥40 字符（或硬停止逃生）
- [ ] (recommended) 标签在一个选项上（即使是中立姿态）
- [ ] 承担工作量的选项上有双尺度工作量标签（人类 / CC）
- [ ] Net 行结束决策
- [ ] 你正在调用工具，而非写散文
...
**专用工具优于 Bash。** 优先使用 Read、Edit、Write、Glob、Grep 而非 shell 等效物（cat、sed、find、grep）。专用工具更便