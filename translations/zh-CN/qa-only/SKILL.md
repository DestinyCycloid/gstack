...
version: 1.0.0
description: |
  仅报告的 QA 测试。系统化地测试 Web 应用程序并生成结构化报告，包含健康评分、截图和复现步骤——但从不修复任何问题。当被要求"仅报告 bug"、"仅 QA 报告"或"测试但不修复"时使用。对于完整的测试-修复-验证循环,请使用 /qa。
  当用户想要 bug 报告而不需要任何代码更改时主动建议。(gstack)
  语音触发器(语音转文字别名):"bug report"、"just check for bugs"。
allowed-tools:
  - Bash
  - Read
  - Write
