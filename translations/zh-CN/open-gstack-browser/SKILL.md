...
description: |
  ...
  当用户说"打开 gstack 浏览器"、"启动浏览器"、"连接 chrome"、
  "打开 chrome"、"真实浏览器"、"启动 chrome"、"侧边面板"或"控制我的浏览器"时使用。
  语音触发词（语音转文字别名）："给我看浏览器"。
triggers:
  - open gstack browser
  - launch chromium
  - show me the browser
...
> 此项目在 `.claude/skills/gstack/` 中内嵌了 gstack。内嵌已弃用。
> 迁移到团队模式？
...
**规则：** 如果你的工具列表中有任何 `mcp__*__AskUserQuestion` 变体，优先使用它。宿主可能通过 `--disallowedTools AskUserQuestion` 禁用原生 AUQ（Conductor 默认这样做）并通过其 MCP 变体路由；在那里调用原生会静默失败。相同的问题/选项结构；相同的决策简报格式适用。
...
优缺点：使用 ✅ 和 ❌。当选择是真实的时，每个选项至少 2 个优点和 1 个缺点；每个要点至少 40 字符。单向/破坏性确认的硬停止逃生：`✅ No cons — this is a hard-stop choice`。

中立姿态：`Recommendation: <default> — this is a taste call, no strong preference either way`；`(recommended)` 保留在默认选项上以供 AUTO_DECIDE 使用。

双向努力：当选项涉及努力时，标记人类团队和 CC+gstack 时间，例如 `(human: ~2 days / CC: ~15 min)`。在决策时使 AI 压缩可见。
...
**重操作前思考。** 对于复杂操作（重构、迁移、非平凡的新功能），在执行前简要说明你的方法。这让用户可以廉价地纠正方向，而不是在进行中。
