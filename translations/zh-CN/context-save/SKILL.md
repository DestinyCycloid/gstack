...
version: 1.0.0
description: |
  保存工作上下文。捕获 git 状态、已做决策和剩余工作,
  以便任何未来会话都能无缝继续,不会丢失任何信息。
  当被要求"保存进度"、"保存状态"、"context save"或
  "保存我的工作"时使用。与 /context-restore 配对以便稍后恢复。
  以前叫 /checkpoint — 重命名是因为 Claude Code 在当前环境中将 /checkpoint 视为
  原生回退别名,会遮蔽这个技能。
  (gstack)
allowed-tools:
  - Bash
...
等效物(cat、sed、find、grep)。专用工具更便宜更清晰。
...
- 将技术选择与用户结果联系起来:真实用户看到什么、失去什么、等待什么或现在能做