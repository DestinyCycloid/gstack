...
version: 1.0.0
description: |
  设计散弹枪:生成多个 AI 设计变体,打开对比面板,
  收集结构化反馈并迭代。可随时运行的独立设计探索工具。使用场景:"探索设计"、"给我看选项"、"设计变体"、
  "视觉头脑风暴"或"我不喜欢这个外观"。
  当用户描述 UI 功能但尚未看到可能的样子时,主动建议使用。(gstack)
triggers:
  - 探索设计变体
  - 给我看设计选项
  - 视觉设计头脑风暴
...
优缺点:使用 ✅ 和 ❌。当选择是真实的时,每个选项至少 2 个优点和 1 个缺点;每个要点至少 40 个字符。单向/破坏性确认的硬停止转义:`✅ 无缺点 — 这是一个硬停止选择`。
...
隐私停止门:如果输出显示 `BRAIN_SYNC: off`,`gbrain_sync_mode_prompted` 是 `false`,并且 gbrain 在 PATH 上或 `gbrain doctor --fast --json` 有效,询问一次:
...
**待办事项列表纪律。**在处理多步骤计划时,在完成每个任务时单独标记为完成。不要在最后批量完成。如果任务被证明是不必要的,用一行原因标记为跳过。

**在重操作之前思考。**对于复杂操作(重构、迁移、非平凡的新功能),在执行前简要说明你的方法。这让用户可以廉价地纠正方向,而不是在飞行中。
