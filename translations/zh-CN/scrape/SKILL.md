...
version: 1.0.0
description: |
  从网页拉取数据。对新意图的首次调用通过 $B 原语原型化流程并返回 JSON。对匹配意图的后续调用
  路由到编码化的浏览器技能,在约 200ms 内返回。只读 — 对于变更流程(表单填写、点击、提交),
  请使用 /automate。
  当被要求"抓取"、"从...获取数据"、"拉取"、"从...提取"或"...上有什么"时使用。(gstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
功能发现,每个会话最多一次提示:
- 缺少 `~/.claude/skills/gstack/.feature-prompted-continuous-checkpoint`:使用 AskUserQuestion 询问持续检查点自动提交。如果接受,运行 `~/.claude/skills/gstack/bin/gstack-config set checkpoint_mode continuous`。始终 touch 标记。
