...
version: 1.0.0
description: |
  使用 browse 守护进程进行性能回归检测。为页面加载时间、核心 Web 指标和资源大小建立基线。
  在每个 PR 上进行前后对比。跟踪性能趋势。
  使用场景："performance"、"benchmark"、"page speed"、"lighthouse"、"web vitals"、
  "bundle size"、"load time"。(gstack)
  语音触发器(语音转文本别名)："speed test"、"check performance"。
triggers:
...
这只在每个项目中发生一次。如果 `HAS_ROUTING` 为 `yes` 或 `ROUTING_DECLINED` 为 `true` 则跳过。
...
**待办事项列表纪律。**在执行多步骤计划时,在完成每个任务时单独标记为完成。不要在最后批量完成。如果某个任务被证明是不必要的,标记为跳过并附上一行原因。
...
直接、具体、开发者对开发者。说明文件、函数、命令和用户可见的影响。没有填充词。
...
在计划模式下 ExitPlanMode 之前:如果计划文件缺少 `## GSTACK REVIEW REPORT`,运行 `~/.claude/skills/gstack/bin/gstack-review-read` 并追加标准的运行/状态/发现表。如果是 `NO_REVIEWS` 或为空,追加一个 5 行占位符,判定为"NO REVIEWS YET — run `/autoplan`"。如果存在更丰富的报告,跳过。
