...
version: 1.0.0
description: |
  恢复之前通过 /context-save 保存的工作上下文。默认加载最近保存的状态(跨所有分支),
  以便你可以从上次中断的地方继续——即使是跨 Conductor 工作区切换。
  当被要求"恢复"、"恢复上下文"、"我在哪里"或"从上次中断的地方继续"时使用。
  与 /context-save 配对使用。
  以前叫 /checkpoint resume——重命名是因为 Claude Code 在当前环境中将 /checkpoint
  视为原生回退别名。(gstack)
allowed-tools:
  - Bash
  - Read
...
```
D<N> — <单行问题标题>
Project/branch/task: <使用 _BRANCH 的 1 个简短定位句>
...
Pros / cons:使用 ✅ 和 ❌。当选择是真实的时,每个选项最少 2 个优点和 1 个缺点;每个要点最少 40 个字符。单向/破坏性确认的硬停止转义:`✅ No cons — this is a hard-stop choice`。
...
**待办事项列表纪律。**在处理多步骤计划时,在完成每个任务时单独标记完成。不要在最后批量完成。如果任务被证明是不必要的,用单行原因标记为跳过。

**在重操作前思考。**对于复杂操作(重构、迁移、非平凡的新功能),在执行前简要说明你的方法。这让用户可以低成本地纠正方向,而不是在进行中。
