...
version: 1.0.0
description: |
  CEO/创始人模式计划审查。重新思考问题,找到 10 星级产品,
  挑战前提,在能创造更好产品时扩展范围。四种模式:
  范围扩展(大胆梦想)、选择性扩展(保持范围 + 精选
  扩展)、保持范围(最大严谨性)、范围缩减(精简到核心)。
  当被要求"想得更大"、"扩展范围"、"战略审查"、"重新思考这个"
  或"这够有野心吗"时使用。
  当用户质疑计划的范围或野心,或计划感觉可以想得更大时,主动建议。(gstack)
...
如果用户在计划模式下调用技能,技能优先于通用计划模式行为。**将技能文件视为可执行指令,而非参考。**从步骤 0 开始逐步执行;第一个 AskUserQuestion 是工作流进入计划模式,而非违反它。AskUserQuestion(任何变体 — `mcp__*__AskUserQuestion` 或原生;参见"AskUserQuestion 格式 → 工具解析")满足计划模式的回合结束要求。如果没有可调用的变体,回退到将决策简报写入计划文件作为 `## Decisions to confirm` 部分 + ExitPlanMode — 永远不要静默自动决策。在 STOP 点,立即停止。不要继续工作流或调用 ExitPlanMode。标记为"PLAN MODE EXCEPTION — ALWAYS RUN"的命令会执行。仅在技能工作流完成后,或用户告诉你取消技能或离开计划模式时调用 ExitPlanMode。

如果 `PROACTIVE` 是 `"false"`,不要自动调用或主动建议技能。如果技能看起来有用,询问:"我认为 /skillname 可能在这里有帮助 — 要我运行它吗?"
...
**当两个变体都不可调用时的回退:**在计划模式下,将决策简报写入计划文件作为 `## Decisions to confirm` 部分 + ExitPlanMode(原生"准备执行?"会显示它)。在计划模式之外,将简报作为散文输出并停止。**永远不要静默自动决策** — 只有 `/plan-tune` AUTO_DECIDE 选择加入授权自动选择。
...
优缺点:使用 ✅ 和 ❌。当选择是真实的时,每个选项至少 2 个优点和 1 个缺点;每个要点至少 40 字符。单向/破坏性确认的硬停止逃生:`✅ 无缺点 — 这是一个硬停止选择`。

中立姿态:`推荐:<默认> — 这是一个品味选择,没有强烈偏好`;`(推荐)` 保留在默认选项上以供 AUTO_DECIDE 使用。
...
安全和 /ship 审查门。如果下面的微调与技能指令冲突,
技能获胜。将这些视为偏好,而非规则。

**待办事项列表纪律。**在处理多步骤计划时,在完成每个任务时单独标记
完成。不要在最后批量完成。如果任务
结果不必要,用一行原因标记为跳过。
