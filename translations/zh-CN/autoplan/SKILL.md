I'll translate this Markdown document from English to Chinese (zh-CN) while preserving all formatting, code blocks, and technical terms.
...
description: |
  ...
  并使用 6 个决策原则按顺序运行它们并自动做出决策。在最终批准关卡
  展示品味决策(接近的方案、边界范围、codex 分歧)。一个命令,输出完全审查的计划。
  当被要求"自动审查"、"autoplan"、"运行所有审查"、"自动审查此计划"
  或"为我做决策"时使用。
  当用户有计划文件并希望运行完整审查流程而不回答 15-30 个中间问题时,
  主动建议使用。(gstack)
  语音触发器(语音转文字别名):"auto plan"、"automatic review"。
...
如果输出显示 `UPGRADE_AVAILABLE <old> <new>`:读取 `~/.claude/skills/gstack/gstack-upgrade/SKILL.md` 并遵循"内联升级流程"(如果配置了则自动升级,否则使用 4 个选项的 AskUserQuestion,如果拒绝则写入暂缓状态)。
...
> v1 提示更简单:首次使用术语注释、以结果为框架的问题、更短的文本。保持默认还是恢复简洁模式?
...
ELI10 始终存在,使用简单英语,而非函数名。推荐始终存在。保留 `(推荐)` 标签;AUTO_DECIDE 依赖于它。

完整性:仅当选项在覆盖范围上不同时使用 `Completeness: N/10`。10 = 完整,7 = 快乐路径,3 = 捷径。如果选项在类型上不同,写:` 注意:选项在类型上不同,而非覆盖范围 — 无完整性评分。`

优缺点:使用 ✅ 和 ❌。当选择是真实的时,每个选项至少 2 个优点和 1 个缺点;每个要点至少 40 个字符。单向/破坏性确认的硬停止逃逸:`✅ 无缺点 — 这是一个硬停止选择`。

中立姿态:`推荐:<默认> — 这是一个品味决定,没有强烈偏好`;`(推荐)` 保留在默认选项上以供 AUTO_DECIDE 使用。
...
隐私停止关卡:如果输出显示 `BRAIN_SYNC: off`,`gbrain_sync_mode_prompted` 是 `false`,且 gbrain 在 PATH 上或 `gbrain doctor --fast --json` 有效,询问一次:
