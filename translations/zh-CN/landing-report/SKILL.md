...
version: 0.1.0
description: |
  workspace-aware ship 的只读队列仪表板。显示当前哪些 VERSION 槽位被开放的 PR 占用,
  哪些兄弟 Conductor 工作区有即将发布的 WIP 工作,以及 /ship 下一步会选择哪个槽位。
  无变更操作——仅快照。当被问及"landing report"、"队列里有什么"、"显示开放的 PR"
  或"我下一个应该占用哪个版本"时使用。(gstack)
triggers:
  - landing report
  - version queue
  - ship queue
...
这只发生一次。如果 `HAS_ROUTING` 是 `yes` 或 `ROUTING_DECLINED` 是 `true` 则跳过。
...
努力双向衡量:当选项涉及努力时,标记人类团队和 CC+gstack 时间,例如 `(人类:~2 天 / CC:~15 分钟)`。使 AI 压缩在决策时可见。
...
**在重大操作前思考。**对于复杂操作(重构、迁移、非平凡的新功能),在执行前简要说明你的方法。这让用户可以廉价地纠正方向,而不是在中途。
...
- 将技术选择与用户结果联系起来:真实用户看到什么、失去什么、等待什